...
dedisp_error update_scrunch_list(dedisp_plan plan) 
{
  ...
  for (int ii = 0; ii < plan->device_count; ii++)
    {
      error = dedisp_set_device(plan->gpu_ids[ii]);
      if (error != DEDISP_NO_ERROR)
	{
  ...
  *plan_ = 0;
  ...
  if( cudaGetLastError() != cudaSuccess ) {
	...
	//Currently always returns DEDISP_NO_ERROR
        //I already check for requested devices and
	//IDs in the peasoup main function
	...
	for (int ii = 0; ii < plan->device_count; ii++) 
	  {
	    err = dedisp_set_device(plan->gpu_ids[ii]);		// set device to one of the selected IDs
	    if (err != DEDISP_NO_ERROR) 
	      {
		dedisp_destroy_plan(plan);
		return err;
                                     ...
                                     dedisp_float ti, dedisp_float tol)
{
	...
	for (int ii = 0; ii < plan->device_count; ii++)
          {
            err = dedisp_set_device(plan->gpu_ids[ii]);
            if (err != DEDISP_NO_ERROR)
              {
                dedisp_destroy_plan(plan);
                return err;
